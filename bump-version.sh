# author: Vsevolod Tolstopyatov <https://github.com/qwwdfsad>
...
result=$(find ./ -type f \( -iname \*.properties -o -iname \*.md \) | grep -v "\.gradle" | grep -v "build" | xargs -I '{}' grep -H "$escaped_old_version" '{}' | grep -v CHANGELOG.md)
if [[ -z "$result" ]]; then
    echo "Done"
else
